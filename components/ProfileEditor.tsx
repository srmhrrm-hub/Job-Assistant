...
export const ProfileEditor: React.FC<ProfileEditorProps> = ({ 
  profiles, activeProfileId, onSelectProfile, onCreateProfile, onDeleteProfile, onUpdateProfile, lang
}) => {
  ...
  const handleDeleteActive = () => {
    ...
    if (confirm(`${t(lang, 'profile_delete_confirm')} "${activeProfile.name}" ?`)) {
        onDeleteProfile(activeProfile.id);
    }
  };

  if (!activeProfile) return <div className="p-10 text-center">...</div>;
    ...
    <div className="flex h-full bg-slate-950">
      ...
      <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0">
        <div className="p-4 border-b border-slate-800">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">{t(lang, 'profile_title')}</h2>
          ...
          {isCreating ? (
            <form onSubmit={handleCreateSubmit} className="mb-2 animate-fade-in">
              ...
              <div className="flex gap-2 text-xs">
                ...
                <button type="button" onClick={() => setIsCreating(false)} className="text-slate-500 hover:text-white">X</button>
              </div>
            </form>
          ) : (
            <button 
            ...
            <div className="flex-1">
               {isRenaming ? (
                   <div className="flex items-center gap-2">
                       ...
                       />
                       <button onClick={handleSaveRename} className="bg-blue-600 hover:bg-blue-500 text-white p-2 rounded-lg">OK</button>
                   </div>
               ) : (
               ...
               <p className="text-slate-400 text-sm mt-1">{t(lang, 'profile_save_auto')}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="flex justify-between text-sm font-semibold text-blue-400 uppercase tracking-wider">
                <span>{t(lang, 'profile_source_cv')}</span>
              </label>
              <textarea
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                placeholder={t(lang, 'profile_source_letter_placeholder')}
              />
            </div>
          </div>
